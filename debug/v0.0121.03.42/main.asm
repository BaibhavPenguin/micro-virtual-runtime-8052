...
;	assignBit
	clr	_is_recieved
;	src/main.c:12: wr_pointer = &prog_buffer[0];
	mov	_wr_pointer,#_prog_buffer
;	src/main.c:13: rd_pointer = &prog_buffer[0];
	mov	_rd_pointer,#_prog_buffer
;	src/main.c:14: } 
	ret
;------------------------------------------------------------
;Allocation info for local variables in function 'init_timer'
;------------------------------------------------------------
;	src/main.c:16: void init_timer(void){
;	-----------------------------------------
;	 function init_timer
;	-----------------------------------------
_init_timer:
;	src/main.c:18: T2CON = 0x00;
	mov	_T2CON,#0x00
;	src/main.c:19: RCAP2H = timer_h;
	mov	_RCAP2H,#0x4c
;	src/main.c:20: RCAP2L = timer_l;
	mov	_RCAP2L,#0x00
;	src/main.c:21: TH2 = timer_h;
	mov	_TH2,#0x4c
;	src/main.c:22: TL2 = timer_l;
	mov	_TL2,#0x00
;	src/main.c:23: TR2 = 1;
;	assignBit
	setb	_TR2
...
;------------------------------------------------------------
;	src/main.c:29: void init_serial(void){
;	-----------------------------------------
;	 function init_serial
;	-----------------------------------------
_init_serial:
;	src/main.c:31: TMOD = 0x20;				//Set TMOD to Use Timer 1 in 8 Bit Auto Reload
	mov	_TMOD,#0x20
;	src/main.c:32: TH1 = serial_baud_timer;	//Set Reload value
	mov	_TH1,#0xfd
;	src/main.c:33: TL1 = serial_baud_timer;	//Set Initial Value
	mov	_TL1,#0xfd
;	src/main.c:34: TR1 = 1;					//Start Timer 1
;	assignBit
	setb	_TR1
;	src/main.c:35: SCON = 0x50;				//Serial Mode 1 , 8 Bit UART
	mov	_SCON,#0x50
;	src/main.c:36: TI = 0;
;	assignBit
	clr	_TI
...
;------------------------------------------------------------
;	src/main.c:40: void uart_send(char dat){
;	-----------------------------------------
;	 function uart_send
;	-----------------------------------------
_uart_send:
	mov	_SBUF,dpl
;	src/main.c:42: while(!TI);
00101$:
;	src/main.c:43: TI = 0;
;	assignBit
	jbc	_TI,00118$
...
;------------------------------------------------------------
;	src/main.c:48: void echo(void){
;	-----------------------------------------
;	 function echo
;	-----------------------------------------
_echo:
;	src/main.c:49: while(!is_recieved);
00101$:
	jnb	_is_recieved,00101$
;	src/main.c:50: switch(*rd_pointer){
	mov	r1,_rd_pointer
	mov	ar7,@r1
	cjne	r7,#0x0d,00105$
;	src/main.c:52: uart_send('\r');uart_send('\n');
	mov	dpl, #0x0d
	lcall	_uart_send
	mov	dpl, #0x0a
	lcall	_uart_send
;	src/main.c:53: default:
00105$:
;	src/main.c:54: uart_send(*rd_pointer);
	mov	r1,_rd_pointer
	mov	dpl,@r1
	lcall	_uart_send
;	src/main.c:56: rd_pointer++;
	inc	_rd_pointer
;	src/main.c:57: is_recieved = 0;
;	assignBit
	clr	_is_recieved
;	src/main.c:59: }
	ret
...
;------------------------------------------------------------
;	src/main.c:65: void delay_ms_500(void){
;	-----------------------------------------
...
;------------------------------------------------------------
;	src/main.c:73: void ascii_to_hex(void){
;	-----------------------------------------
...
;------------------------------------------------------------
;	src/main.c:82: void hex_to_ascii(void){
;	-----------------------------------------
...
;------------------------------------------------------------
;	src/main.c:90: void print_boot_message(void){
;	-----------------------------------------
;	 function print_boot_message
;	-----------------------------------------
_print_boot_message:
;	src/main.c:91: temp0 = 0;
	mov	_temp0,#0x00
;	src/main.c:92: while(temp0 != len_boot_message){
00101$:
	mov	a,#0x36
	cjne	a,_temp0,00118$
	sjmp	00103$
00118$:
;	src/main.c:93: uart_send(boot_message[temp0]);
	mov	a,_temp0
...
;------------------------------------------------------------
;	src/main.c:99: void print_ready_message(void){
;	-----------------------------------------
;	 function print_ready_message
;	-----------------------------------------
_print_ready_message:
;	src/main.c:100: temp0 = 0;
	mov	_temp0,#0x00
;	src/main.c:101: while(temp0 != len_ready_message){
00101$:
	mov	a,#0x07
	cjne	a,_temp0,00118$
	sjmp	00103$
00118$:
;	src/main.c:102: uart_send(ready_msg[temp0]);
	mov	a,_temp0
	mov	dptr,#_ready_msg
	movc	a,@a+dptr
	mov	dpl,a
	lcall	_uart_send
;	src/main.c:103: temp0++;
	mov	a,_temp0
	inc	a
	mov	_temp0,a
	sjmp	00101$
00103$:
;	src/main.c:105: temp0 = 0;
	mov	_temp0,#0x00
;	src/main.c:106: }
	ret
;------------------------------------------------------------
...
;------------------------------------------------------------
;	src/main.c:108: void clear_terminal(void){
;	-----------------------------------------
;	 function clear_terminal
;	-----------------------------------------
_clear_terminal:
;	src/main.c:111: uart_send(0x1B);uart_send(0x63); //Full Reset Terminal
	mov	dpl, #0x1b
	lcall	_uart_send
	mov	dpl, #0x63
;	src/main.c:112: }
...
;------------------------------------------------------------
;	src/main.c:116: void Serial_ISR(void) __interrupt(4)
;	-----------------------------------------
;	 function Serial_ISR
;	-----------------------------------------
_Serial_ISR:
	...
	push	psw
	mov	psw,#0x00
;	src/main.c:118: if(RI){
	jnb	_RI,00103$
;	src/main.c:119: *wr_pointer = SBUF;
	mov	a,_wr_pointer
	mov	r0,a
	mov	@r0,_SBUF
;	src/main.c:120: is_recieved = 1;
;	assignBit
	setb	_is_recieved
;	src/main.c:121: wr_pointer++;
	inc	_wr_pointer
;	src/main.c:122: RI = 0;
;	assignBit
	clr	_RI
...
;------------------------------------------------------------
;	src/main.c:127: void Timer2_ISR(void) __interrupt(5)
;	-----------------------------------------
;	 function Timer2_ISR
;	-----------------------------------------
_Timer2_ISR:
	push	acc
;	src/main.c:129: system_tick++;
	mov	a,_system_tick
	inc	a
	mov	_system_tick,a
;	src/main.c:130: }
...
;------------------------------------------------------------
;	src/main.c:133: void main(void){
;	-----------------------------------------
;	 function main
;	-----------------------------------------
_main:
;	src/main.c:134: SP = hw_stack; 		//initiaslze Stack Pointer;
	mov	_SP,#0x60
;	src/main.c:135: init_sys();
	lcall	_init_sys
;	src/main.c:136: init_serial();
	lcall	_init_serial
;	src/main.c:140: clear_terminal();
	lcall	_clear_terminal
;	src/main.c:141: print_boot_message();
	lcall	_print_boot_message
;	src/main.c:142: print_ready_message();
