...
;	src/main.c:79: temp0 = 0;
	mov	_temp0,#0x00
;	src/main.c:80: while(temp0 != len_boot_message){
00101$:
	mov	a,#0x36
	cjne	a,_temp0,00118$
	sjmp	00103$
...
;	src/main.c:88: temp0 = 0;
	mov	_temp0,#0x00
;	src/main.c:89: while(temp0 != len_ready_message){
00101$:
	mov	a,#0x07
	cjne	a,_temp0,00118$
	sjmp	00103$
...
;	-----------------------------------------
_clear_terminal:
;	src/main.c:99: uart_send(0x1B);uart_send(0x63); //Full Reset Terminal
	mov	dpl, #0x1b
	lcall	_uart_send
	mov	dpl, #0x63
;	src/main.c:100: }
...
;------------------------------------------------------------
;	src/main.c:104: void Serial_ISR(void) __interrupt(4)
;	-----------------------------------------
;	 function Serial_ISR
;	-----------------------------------------
_Serial_ISR:
	...
	push	psw
	mov	psw,#0x00
;	src/main.c:106: if(RI){
	jnb	_RI,00103$
;	src/main.c:107: *wr_pointer = SBUF;
	mov	dptr,#_wr_pointer
	movx	a,@dptr
	mov	r1,a
	mov	@r1,_SBUF
;	src/main.c:108: is_recieved = 1;
;	assignBit
	setb	_is_recieved
...
;------------------------------------------------------------
;	src/main.c:115: void Timer2_ISR(void) __interrupt(5)
;	-----------------------------------------
;	 function Timer2_ISR
;	-----------------------------------------
_Timer2_ISR:
	push	acc
;	src/main.c:117: system_tick++;
	mov	a,_system_tick
	inc	a
	mov	_system_tick,a
;	src/main.c:118: }
...
;	-----------------------------------------
_main:
;	src/main.c:122: SP = hw_stack; 		//initiaslze Stack Pointer;
	mov	_SP,#0x60
;	src/main.c:123: init_sys();
	lcall	_init_sys
;	src/main.c:124: init_serial();
	lcall	_init_serial
;	src/main.c:128: clear_terminal();
	lcall	_clear_terminal
;	src/main.c:129: print_boot_message();
	lcall	_print_boot_message
;	src/main.c:130: print_ready_message();
