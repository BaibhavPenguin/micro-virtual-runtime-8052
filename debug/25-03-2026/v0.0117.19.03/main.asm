...
;------------------------------------------------------------
;	src/main.c:85: void Serial_ISR(void) __interrupt(4)
;	-----------------------------------------
;	 function Serial_ISR
;	-----------------------------------------
_Serial_ISR:
...
;	src/main.c:87: if(RI){
	jnb	_RI,00103$
;	src/main.c:88: *wr_pointer = SBUF;
	mov	dptr,#_wr_pointer
	movx	a,@dptr
	mov	r1,a
	mov	@r1,_SBUF
;	src/main.c:89: is_recieved = 1;
;	assignBit
	setb	_is_recieved
;	src/main.c:90: wr_pointer++;
	mov	dptr,#_wr_pointer
	mov	a,r1
	inc	a
	movx	@dptr,a
...
;------------------------------------------------------------
;	src/main.c:95: void Timer2_ISR(void) __interrupt(5)
;	-----------------------------------------
;	 function Timer2_ISR
;	-----------------------------------------
_Timer2_ISR:
	push	acc
;	src/main.c:97: system_tick++;
	mov	a,_system_tick
	inc	a
	mov	_system_tick,a
;	src/main.c:98: }
...
;	-----------------------------------------
_main:
;	src/main.c:102: SP = hw_stack; 		//initiaslze Stack Pointer;
	mov	_SP,#0x60
;	src/main.c:103: init_sys();
	lcall	_init_sys
;	src/main.c:104: init_serial();
	lcall	_init_serial
;	src/main.c:105: while(1){
00102$:
;	src/main.c:106: uart_send('H');
	mov	dpl, #0x48
	lcall	_uart_send
;	src/main.c:107: uart_send('e');
	mov	dpl, #0x65
	lcall	_uart_send
;	src/main.c:108: uart_send('l');
	mov	dpl, #0x6c
	lcall	_uart_send
;	src/main.c:109: uart_send('l');
	mov	dpl, #0x6c
	lcall	_uart_send
;	src/main.c:110: uart_send('o');
	mov	dpl, #0x6f
	lcall	_uart_send
;	src/main.c:111: uart_send(' ');
	mov	dpl, #0x20
	lcall	_uart_send
;	src/main.c:113: }
	sjmp	00102$
