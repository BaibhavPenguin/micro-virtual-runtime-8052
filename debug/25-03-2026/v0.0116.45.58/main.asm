...
;	include/uvr_variable.h:21: volatile unsigned char __data __at (0x38) echo_buffer = 0;
...
;--------------------------------------------------------
	.area CSEG    (CODE)
;------------------------------------------------------------
;Allocation info for local variables in function 'init_mem'
;------------------------------------------------------------
;	src/main.c:6: void init_mem(void){
;	-----------------------------------------
;	 function init_mem
;	-----------------------------------------
_init_mem:
...
;------------------------------------------------------------
;	src/main.c:12: void init_timer(void){
;	-----------------------------------------
;	 function init_timer
;	-----------------------------------------
_init_timer:
...
;------------------------------------------------------------
;	src/main.c:17: void init_serial(void){
;	-----------------------------------------
;	 function init_serial
;	-----------------------------------------
_init_serial:
;	src/main.c:19: TMOD = 0x20;				//Set TMOD to Use Timer 1 in 8 Bit Auto Reload
	mov	_TMOD,#0x20
;	src/main.c:20: TH1 = serial_baud_timer;	//Set Reload value
	mov	_TH1,#0xfd
;	src/main.c:21: TL1 = serial_baud_timer;	//Set Initial Value
	mov	_TL1,#0xfd
;	src/main.c:22: TR1 = 1;					//Start Timer 1
;	assignBit
	setb	_TR1
;	src/main.c:23: SCON = 0x50;				//Serial Mode 1 , 8 Bit UART
	mov	_SCON,#0x50
;	src/main.c:26: TI = 0;
;	assignBit
	clr	_TI
...
;------------------------------------------------------------
;	src/main.c:30: void put_char(char dat){
;	-----------------------------------------
;	 function put_char
;	-----------------------------------------
_put_char:
	mov	r7, dpl
;	src/main.c:31: while(serial_busy);
00101$:
	jb	_serial_busy,00101$
;	src/main.c:32: serial_busy = 1;
;	assignBit
	setb	_serial_busy
;	src/main.c:33: SBUF = dat;
	mov	_SBUF,r7
;	src/main.c:34: }
	ret
;------------------------------------------------------------
...
;------------------------------------------------------------
;	src/main.c:46: void delay_ms_500(void){
;	-----------------------------------------
...
;------------------------------------------------------------
;	src/main.c:54: void ascii_to_hex(void){
;	-----------------------------------------
...
;------------------------------------------------------------
;	src/main.c:63: void hex_to_ascii(void){
;	-----------------------------------------
...
;------------------------------------------------------------
;	src/main.c:71: void print_boot_message(void){
;	-----------------------------------------
;	 function print_boot_message
;	-----------------------------------------
_print_boot_message:
;	src/main.c:73: put_char('D');put_char('e');put_char('b');put_char('u');put_char('g');put_char('\n');
	mov	dpl, #0x44
	lcall	_put_char
	mov	dpl, #0x65
	lcall	_put_char
...
;------------------------------------------------------------
;	src/main.c:79: void Serial_ISR(void) __interrupt(4)
;	-----------------------------------------
;	 function Serial_ISR
;	-----------------------------------------
_Serial_ISR:
...
;	assignBit
	...
	sjmp	00102$
00119$:
;	src/main.c:83: serial_busy = 0;
;	assignBit
	clr	_serial_busy
00102$:
;	src/main.c:86: if(RI){
	jnb	_RI,00105$
;	src/main.c:87: *wr_pointer = SBUF;
	mov	dptr,#_wr_pointer
	movx	a,@dptr
	mov	r1,a
	mov	@r1,_SBUF
;	src/main.c:88: echo_buffer = SBUF;
	mov	_echo_buffer,_SBUF
;	src/main.c:89: wr_pointer++;
	mov	a,r1
	inc	a
	movx	@dptr,a
;	src/main.c:90: RI = 0;
...
;	-----------------------------------------
_main:
;	src/main.c:97: SP = hw_stack; 		//initiaslze Stack Pointer;
	mov	_SP,#0x60
;	src/main.c:98: init_mem();
	lcall	_init_mem
;	src/main.c:99: init_serial();
	lcall	_init_serial
