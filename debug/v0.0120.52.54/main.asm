...
;	-----------------------------------------
_echo:
;	src/main.c:49: rd_pointer = &prog_buffer[0];
	mov	_rd_pointer,#_prog_buffer
;	src/main.c:50: while(!is_recieved);
00101$:
	jnb	_is_recieved,00101$
;	src/main.c:51: uart_send(*rd_pointer);
	mov	r1,_rd_pointer
	mov	dpl,@r1
	lcall	_uart_send
;	src/main.c:52: rd_pointer++;
	inc	_rd_pointer
;	src/main.c:53: is_recieved = 0;
;	assignBit
	clr	_is_recieved
;	src/main.c:55: }
	ret
...
;------------------------------------------------------------
;	src/main.c:61: void delay_ms_500(void){
;	-----------------------------------------
...
;------------------------------------------------------------
;	src/main.c:69: void ascii_to_hex(void){
;	-----------------------------------------
...
;------------------------------------------------------------
;	src/main.c:78: void hex_to_ascii(void){
;	-----------------------------------------
...
;------------------------------------------------------------
;	src/main.c:86: void print_boot_message(void){
;	-----------------------------------------
;	 function print_boot_message
;	-----------------------------------------
_print_boot_message:
;	src/main.c:87: temp0 = 0;
	mov	_temp0,#0x00
;	src/main.c:88: while(temp0 != len_boot_message){
00101$:
	mov	a,#0x36
	cjne	a,_temp0,00118$
	sjmp	00103$
00118$:
;	src/main.c:89: uart_send(boot_message[temp0]);
	mov	a,_temp0
...
;------------------------------------------------------------
;	src/main.c:95: void print_ready_message(void){
;	-----------------------------------------
...
;	src/main.c:96: temp0 = 0;
	mov	_temp0,#0x00
;	src/main.c:97: while(temp0 != len_ready_message){
00101$:
	mov	a,#0x07
	cjne	a,_temp0,00118$
	sjmp	00103$
00118$:
;	src/main.c:98: uart_send(ready_msg[temp0]);
	mov	a,_temp0
	mov	dptr,#_ready_msg
	movc	a,@a+dptr
	mov	dpl,a
...
;------------------------------------------------------------
;	src/main.c:104: void clear_terminal(void){
;	-----------------------------------------
;	 function clear_terminal
;	-----------------------------------------
_clear_terminal:
;	src/main.c:107: uart_send(0x1B);uart_send(0x63); //Full Reset Terminal
	mov	dpl, #0x1b
	lcall	_uart_send
	mov	dpl, #0x63
;	src/main.c:108: }
...
_Serial_ISR:
	...
	push	psw
	mov	psw,#0x00
;	src/main.c:114: if(RI){
	jnb	_RI,00103$
;	src/main.c:115: *wr_pointer = SBUF;
	mov	a,_wr_pointer
	mov	r0,a
	mov	@r0,_SBUF
;	src/main.c:116: is_recieved = 1;
;	assignBit
	setb	_is_recieved
;	src/main.c:117: wr_pointer++;
	inc	_wr_pointer
;	src/main.c:118: RI = 0;
;	assignBit
	clr	_RI
...
;------------------------------------------------------------
;	src/main.c:123: void Timer2_ISR(void) __interrupt(5)
;	-----------------------------------------
;	 function Timer2_ISR
;	-----------------------------------------
_Timer2_ISR:
	push	acc
;	src/main.c:125: system_tick++;
	mov	a,_system_tick
	inc	a
	mov	_system_tick,a
;	src/main.c:126: }
...
;	-----------------------------------------
_main:
;	src/main.c:130: SP = hw_stack; 		//initiaslze Stack Pointer;
	mov	_SP,#0x60
;	src/main.c:131: init_sys();
	lcall	_init_sys
;	src/main.c:132: init_serial();
	lcall	_init_serial
;	src/main.c:136: clear_terminal();
	lcall	_clear_terminal
;	src/main.c:137: print_boot_message();
	lcall	_print_boot_message
;	src/main.c:138: print_ready_message();
	lcall	_print_ready_message
;	src/main.c:140: while(1){
00102$:
;	src/main.c:141: echo();
	lcall	_echo
;	src/main.c:143: }
