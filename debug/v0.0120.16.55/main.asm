...
;	-----------------------------------------
_print_boot_message:
;	src/main.c:79: loop_counter = 0;
	mov	_loop_counter,#0x00
;	src/main.c:80: while(loop_counter != 54){
00101$:
	mov	a,#0x36
	cjne	a,_loop_counter,00118$
	sjmp	00103$
00118$:
;	src/main.c:81: uart_send(boot_message[loop_counter]);
	mov	a,_loop_counter
	mov	dptr,#_boot_message
	movc	a,@a+dptr
	mov	dpl,a
	lcall	_uart_send
;	src/main.c:82: loop_counter++;
	mov	a,_loop_counter
	inc	a
	mov	_loop_counter,a
	sjmp	00101$
00103$:
;	src/main.c:84: loop_counter = 0;
	mov	_loop_counter,#0x00
;	src/main.c:85: }
	ret
;------------------------------------------------------------
...
;------------------------------------------------------------
;	src/main.c:89: void Serial_ISR(void) __interrupt(4)
;	-----------------------------------------
;	 function Serial_ISR
;	-----------------------------------------
_Serial_ISR:
...
;	src/main.c:91: if(RI){
	jnb	_RI,00103$
;	src/main.c:92: *wr_pointer = SBUF;
	mov	dptr,#_wr_pointer
	movx	a,@dptr
	mov	r1,a
	mov	@r1,_SBUF
;	src/main.c:93: is_recieved = 1;
;	assignBit
	setb	_is_recieved
;	src/main.c:94: wr_pointer++;
	mov	dptr,#_wr_pointer
	mov	a,r1
	inc	a
	movx	@dptr,a
...
;------------------------------------------------------------
;	src/main.c:100: void Timer2_ISR(void) __interrupt(5)
;	-----------------------------------------
;	 function Timer2_ISR
;	-----------------------------------------
_Timer2_ISR:
	push	acc
;	src/main.c:102: system_tick++;
	mov	a,_system_tick
	inc	a
	mov	_system_tick,a
;	src/main.c:103: }
...
;	-----------------------------------------
_main:
;	src/main.c:107: SP = hw_stack; 		//initiaslze Stack Pointer;
	mov	_SP,#0x60
;	src/main.c:108: init_sys();
	lcall	_init_sys
;	src/main.c:109: init_serial();
	lcall	_init_serial
;	src/main.c:113: print_boot_message();
