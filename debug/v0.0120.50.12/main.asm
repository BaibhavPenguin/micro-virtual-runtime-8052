...
;	-----------------------------------------
_echo:
;	src/main.c:46: rd_pointer = &prog_buffer[0];
	mov	dptr,#_rd_pointer
	mov	a,#_prog_buffer
	movx	@dptr,a
;	src/main.c:47: while(!is_recieved);
00101$:
	jnb	_is_recieved,00101$
;	src/main.c:48: uart_send(*rd_pointer);
	mov	dpl,_prog_buffer
	lcall	_uart_send
;	src/main.c:49: rd_pointer++;
	mov	dptr,#_rd_pointer
	movx	a,@dptr
	add	a, #0x01
	movx	@dptr,a
;	src/main.c:50: is_recieved = 0;
;	assignBit
	clr	_is_recieved
;	src/main.c:52: }
	ret
...
;------------------------------------------------------------
;	src/main.c:58: void delay_ms_500(void){
;	-----------------------------------------
...
;------------------------------------------------------------
;	src/main.c:66: void ascii_to_hex(void){
;	-----------------------------------------
...
;------------------------------------------------------------
;	src/main.c:75: void hex_to_ascii(void){
;	-----------------------------------------
...
;------------------------------------------------------------
;	src/main.c:83: void print_boot_message(void){
;	-----------------------------------------
...
;	src/main.c:84: temp0 = 0;
	mov	_temp0,#0x00
;	src/main.c:85: while(temp0 != len_boot_message){
00101$:
	mov	a,#0x36
	cjne	a,_temp0,00118$
	sjmp	00103$
00118$:
;	src/main.c:86: uart_send(boot_message[temp0]);
	mov	a,_temp0
...
;------------------------------------------------------------
;	src/main.c:92: void print_ready_message(void){
;	-----------------------------------------
...
;	src/main.c:93: temp0 = 0;
	mov	_temp0,#0x00
;	src/main.c:94: while(temp0 != len_ready_message){
00101$:
	mov	a,#0x07
	cjne	a,_temp0,00118$
	sjmp	00103$
00118$:
;	src/main.c:95: uart_send(ready_msg[temp0]);
	mov	a,_temp0
	mov	dptr,#_ready_msg
	movc	a,@a+dptr
	mov	dpl,a
...
;------------------------------------------------------------
;	src/main.c:101: void clear_terminal(void){
;	-----------------------------------------
;	 function clear_terminal
;	-----------------------------------------
_clear_terminal:
;	src/main.c:104: uart_send(0x1B);uart_send(0x63); //Full Reset Terminal
	mov	dpl, #0x1b
	lcall	_uart_send
	mov	dpl, #0x63
;	src/main.c:105: }
...
_Serial_ISR:
	...
	push	psw
	mov	psw,#0x00
;	src/main.c:111: if(RI){
	jnb	_RI,00103$
;	src/main.c:112: *wr_pointer = SBUF;
	mov	dptr,#_wr_pointer
	movx	a,@dptr
	mov	r1,a
	mov	@r1,_SBUF
;	src/main.c:113: is_recieved = 1;
;	assignBit
	setb	_is_recieved
...
;------------------------------------------------------------
;	src/main.c:120: void Timer2_ISR(void) __interrupt(5)
;	-----------------------------------------
;	 function Timer2_ISR
;	-----------------------------------------
_Timer2_ISR:
	push	acc
;	src/main.c:122: system_tick++;
	mov	a,_system_tick
	inc	a
	mov	_system_tick,a
;	src/main.c:123: }
...
;	-----------------------------------------
_main:
;	src/main.c:127: SP = hw_stack; 		//initiaslze Stack Pointer;
	mov	_SP,#0x60
;	src/main.c:128: init_sys();
	lcall	_init_sys
;	src/main.c:129: init_serial();
	lcall	_init_serial
;	src/main.c:133: clear_terminal();
	lcall	_clear_terminal
;	src/main.c:134: print_boot_message();
	lcall	_print_boot_message
;	src/main.c:135: print_ready_message();
