...
;	-----------------------------------------
_clear_terminal:
;	src/main.c:97: uart_send(0x1B);uart_send(0x5B);uart_send(0x32);uart_send(0x4A);
	mov	dpl, #0x1b
	lcall	_uart_send
	mov	dpl, #0x5b
	lcall	_uart_send
...
_Serial_ISR:
	...
	push	psw
	mov	psw,#0x00
;	src/main.c:104: if(RI){
	jnb	_RI,00103$
;	src/main.c:105: *wr_pointer = SBUF;
	mov	dptr,#_wr_pointer
	movx	a,@dptr
	mov	r1,a
	mov	@r1,_SBUF
;	src/main.c:106: is_recieved = 1;
;	assignBit
	setb	_is_recieved
...
;------------------------------------------------------------
;	src/main.c:113: void Timer2_ISR(void) __interrupt(5)
;	-----------------------------------------
;	 function Timer2_ISR
;	-----------------------------------------
_Timer2_ISR:
	push	acc
;	src/main.c:115: system_tick++;
	mov	a,_system_tick
	inc	a
	mov	_system_tick,a
;	src/main.c:116: }
...
;	-----------------------------------------
_main:
;	src/main.c:120: SP = hw_stack; 		//initiaslze Stack Pointer;
	mov	_SP,#0x60
;	src/main.c:121: init_sys();
	lcall	_init_sys
;	src/main.c:122: init_serial();
	lcall	_init_serial
;	src/main.c:126: clear_terminal();
	lcall	_clear_terminal
;	src/main.c:127: print_boot_message();
	lcall	_print_boot_message
;	src/main.c:128: print_ready_message();
